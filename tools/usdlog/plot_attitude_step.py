...
folders = ["tools/usdlog/logs/05_09/att_dist/lilac_wood", 
...
for folder in folders:
    ...
    if axs_id > 2:
        axes[axs_id].set_xlabel("Time (seconds)")  # X label in seconds 

    axs_id += 1
    color_idx += 1

# Add a common y-label on the left side of the figure
fig.text(0.06, 0.5, 'Roll Angle (degrees)', va='center', rotation='vertical', fontsize=12)
...
# Save the figure
plt.savefig("tools/usdlog/roll_commands.svg", format='svg')
